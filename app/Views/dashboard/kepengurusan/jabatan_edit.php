...
<head>
    ...
    <title>Ubah Jabatan Kepengurusan - <?= site_name() ?></title>
...
<body>
    ...
    <main class="main-content">
        ...
        <div class="topbar">
            <div>
                <h1 class="h3 fw-bold mb-1 text-dark">Ubah Jabatan Kepengurusan</h1>
                <p class="text-muted mb-0">Sesuaikan informasi jabatan struktur kepengurusan masjid.</p>
            </div>
        ...
        <div class="panel-card">
            <form action="<?= base_url('dashboard/kepengurusan/jabatan/update/' . esc($jabatan['id'])) ?>" method="post">
                <?= csrf_field() ?>

                <div class="row">
                    <div class="col-md-6 mb-4">
                        <label for="periode_id" class="form-label">Periode Kepengurusan</label>
                        <select class="form-select" id="periode_id" name="periode_id" required>
                            <option value="">-- Pilih Periode --</option>
                            <?php foreach ($periode_list as $periode) : ?>
                                <option value="<?= esc($periode['id']) ?>" <?= old('periode_id', esc($jabatan['periode_id'])) == $periode['id'] ? 'selected' : '' ?>>
                    ...
                    <div class="col-md-6 mb-4">
                        ...
                        <input type="text" class="form-control" id="nama_jabatan" name="nama_jabatan" placeholder="Contoh: Ketua Umum / Anggota Humas / Pembina" value="<?= old('nama_jabatan', esc($jabatan['nama_jabatan'])) ?>" required>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-md-8 mb-4">
                        ...
                        <select class="form-select" id="parent_id" name="parent_id">
                            ...
                            <!-- Diisi dinamis menggunakan JavaScript berdasarkan periode terpilih -->
                        </select>
                    </div>

                    <div class="col-md-4 mb-4">
                        <label for="urutan" class="form-label">Urutan Visual Tampilan</label>
    ...
    <script>
        ...
        const oldParentId = "<?= old('parent_id', esc($jabatan['parent_id'])) ?>";

        function filterParentJabatan() {
            const selectedPeriode = document.getElementById('periode_id').value;
            const parentSelect = document.getElementById('parent_id');
            parentSelect.innerHTML = '<option value="">-- Tanpa Atasan (Puncak Pimpinan) --</option>';
            
            // Kecualikan jabatan ini sendiri agar tidak circular dependency
            const filtered = rawJabatanList.filter(j => j.periode_id === selectedPeriode && j.id !== currentJabatanId);
            filtered.forEach(j => {
        ...
        document.getElementById('periode_id').addEventListener('change', filterParentJabatan);
