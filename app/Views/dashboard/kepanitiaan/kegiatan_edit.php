...
<head>
    ...
    <title>Ubah Kegiatan - <?= site_name() ?></title>
...
<body>
    ...
    <main class="main-content">
        ...
        <div class="topbar">
            <div>
                <h1 class="h3 fw-bold mb-1 text-dark">Ubah Kegiatan</h1>
                <p class="text-muted mb-0">Perbarui rincian data kegiatan masjid.</p>
            </div>
        ...
        <div class="panel-card">
            <form action="<?= base_url('dashboard/kepanitiaan/kegiatan/update/' . esc($kegiatan['id'])) ?>" method="post">
                <?= csrf_field() ?>

                <div class="mb-4">
                    <label for="nama_kegiatan" class="form-label">Nama Kegiatan</label>
                    <input type="text" class="form-control" id="nama_kegiatan" name="nama_kegiatan" placeholder="Contoh: Panitia Ramadhan 1447 H / Tabligh Akbar Akbar" value="<?= old('nama_kegiatan', esc($kegiatan['nama_kegiatan'])) ?>" required>
                </div>

                <div class="row">
                    <div class="col-md-6 mb-4">
                        <label for="tanggal_mulai" class="form-label">Tanggal Mulai Kegiatan</label>
                        <input type="date" class="form-control" id="tanggal_mulai" name="tanggal_mulai" value="<?= old('tanggal_mulai', esc($kegiatan['tanggal_mulai'])) ?>" required>
                    </div>

                    <div class="col-md-6 mb-4">
                        <label for="tanggal_selesai" class="form-label">Tanggal Selesai Kegiatan</label>
                        <input type="date" class="form-control" id="tanggal_selesai" name="tanggal_selesai" value="<?= old('tanggal_selesai', esc($kegiatan['tanggal_selesai'])) ?>" required>
                    </div>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <select class="form-select" id="status" name="status" required>
                        <option value="rencana" <?= old('status', esc($kegiatan['status'])) == 'rencana' ? 'selected' : '' ?>>Rencana</option>
                        <option value="berjalan" <?= old('status', esc($kegiatan['status'])) == 'berjalan' ? 'selected' : '' ?>>Berjalan</option>
                        <option value="selesai" <?= old('status', esc($kegiatan['status'])) == 'selesai' ? 'selected' : '' ?>>Selesai</option>
                        <option value="dibatalkan" <?= old('status', esc($kegiatan['status'])) == 'dibatalkan' ? 'selected' : '' ?>>Dibatalkan</option>
                    </select>
                </div>

                <div class="mb-4">
                    <label for="deskripsi" class="form-label">Deskripsi / Catatan Kegiatan</label>
                    <textarea class="form-control" id="deskripsi" name="deskripsi" rows="4" placeholder="Tuliskan keterangan detail kegiatan (opsional)"><?= old('deskripsi', esc($kegiatan['deskripsi'])) ?></textarea>
                </div>
