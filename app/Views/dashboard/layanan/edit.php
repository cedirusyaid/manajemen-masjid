...
<head>
    ...
    <title>Ubah Layanan - <?= site_name() ?></title>
...
<body>
    ...
    <main class="main-content">
        <div class="d-flex align-items-center justify-content-between mb-4">
            <div>
                <h1 class="h3 fw-bold mb-1 text-dark">Ubah Layanan Jamaah</h1>
                <p class="text-muted mb-0">Perbarui informasi layanan dan konfigurasi kontak.</p>
            </div>
        ...
        <div class="panel-card">
            <form action="<?= base_url('dashboard/layanan/update/' . esc($layanan['id'])) ?>" method="post">
                <?= csrf_field() ?>

                <div class="row">
                    <div class="col-md-8 mb-4">
                        <label for="nama_layanan" class="form-label">Nama Layanan Jamaah</label>
                        <input type="text" class="form-control" id="nama_layanan" name="nama_layanan" placeholder="Contoh: Konsultasi Hukum Waris" value="<?= old('nama_layanan', esc($layanan['nama_layanan'])) ?>" required>
                    </div>

                    <div class="col-md-4 mb-4">
                        <label for="warna_tema" class="form-label">Warna Tema Tampilan</label>
                        <?php $currentWarna = old('warna_tema', $layanan['warna_tema'] ?? 'success'); ?>
                        <select class="form-select" id="warna_tema" name="warna_tema" required>
                            <option value="success" <?= $currentWarna === 'success' ? 'selected' : '' ?>>Hijau (Success)</option>
                            <option value="primary" <?= $currentWarna === 'primary' ? 'selected' : '' ?>>Biru (Primary)</option>
                            <option value="warning" <?= $currentWarna === 'warning' ? 'selected' : '' ?>>Kuning / Emas (Warning)</option>
                            <option value="danger" <?= $currentWarna === 'danger' ? 'selected' : '' ?>>Merah (Danger)</option>
                            <option value="info" <?= $currentWarna === 'info' ? 'selected' : '' ?>>Cyan / Biru Muda (Info)</option>
                            <option value="secondary" <?= $currentWarna === 'secondary' ? 'selected' : '' ?>>Abu-abu (Secondary)</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-md-12 mb-4">
                        ...
                        <textarea class="form-control" id="deskripsi" name="deskripsi" rows="3" placeholder="Jelaskan fasilitas dan bantuan apa yang diberikan dalam layanan ini kepada jemaah..."><?= old('deskripsi', esc($layanan['deskripsi'])) ?></textarea>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-md-4 mb-4">
                        ...
                        <input type="text" class="form-control" id="icon" name="icon" placeholder="fa-solid fa-hand-holding-heart" value="<?= old('icon', esc($layanan['icon'])) ?>">
                    ...
                    <div class="col-md-4 mb-4">
                        ...
                        <input type="text" class="form-control" id="kontak_wa" name="kontak_wa" placeholder="Contoh: 081234567890" value="<?= old('kontak_wa', esc($layanan['kontak_wa'])) ?>">
                    </div>
                    ...
                    <div class="col-md-4 mb-4">
                        ...
                        <select class="form-select" id="pj_personil_id" name="pj_personil_id">
                            ...
                            <?php foreach ($personil_list as $p) : ?>
                                <option value="<?= esc($p['id']) ?>" <?= old('pj_personil_id', $layanan['pj_personil_id']) == $p['id'] ? 'selected' : '' ?>>
                                    <?= esc($p['nama']) ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                ...
                <div class="row">
                    <div class="col-md-8 mb-4">
                        ...
                        <input type="text" class="form-control" id="pesan_wa_default" name="pesan_wa_default" placeholder="Assalamu'alaikum Pengurus..." value="<?= old('pesan_wa_default', esc($layanan['pesan_wa_default'])) ?>">
                    </div>

                    <div class="col-md-2 mb-4">
                        <label for="urutan" class="form-label">Urutan</label>
                        <input type="number" class="form-control" id="urutan" name="urutan" value="<?= old('urutan', esc($layanan['urutan'])) ?>">
                    </div>

                    <div class="col-md-2 mb-4">
                        <label for="status" class="form-label">Status Web</label>
                        <?php $currentStatus = old('status', $layanan['status'] ?? 'aktif'); ?>
