...
<head>
    ...
    <title>Master Layanan Jamaah - <?= site_name() ?></title>
    ...
    <main class="main-content">
        ...
        <div class="topbar">
            <div class="topbar-title">
                <h1>Master Layanan Jamaah</h1>
                <p class="text-muted mb-0">Kelola daftar layanan dan kontak WhatsApp yang tampil di halaman publik.</p>
            </div>
        ...
        <div class="panel-card">
            <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-4 pb-3 border-bottom">
                <div class="d-flex align-items-center gap-2">
                    <span class="badge bg-success bg-opacity-10 text-success border border-success border-opacity-25 px-3 py-2 rounded-pill font-heading">
                        Total <?= count($layanan_list) ?> Jenis Layanan
                    </span>
                </div>
                <div class="d-flex gap-2">
                    <a href="<?= base_url('dashboard/pelayanan') ?>" class="btn btn-outline-secondary px-3 py-2 rounded-3 fw-semibold">
                        <i class="fa-solid fa-clipboard-list me-1"></i> Lihat Buku Catatan Pelayanan
                    </a>
                    <a href="<?= base_url('dashboard/layanan/create') ?>" class="btn-add">
                ...
                <table class="table">
                    <thead>
                        <tr>
                            ...
                            <th>Penanggung Jawab / WA</th>
                            <th class="text-center">Total Pelayanan</th>
                    ...
                    <tbody>
                        <?php if (!empty($layanan_list)) : ?>
                            <?php foreach ($layanan_list as $lay) : ?>
                                <tr>
                                    <td class="text-center fw-bold text-muted"><?= esc($lay['urutan']) ?></td>
                                    <td>
                                        <div class="d-flex align-items-center gap-3">
                                            <div class="p-2.5 rounded-3 bg-<?= esc($lay['warna_tema']) ?> bg-opacity-10 text-<?= esc($lay['warna_tema']) ?> fs-5">
                                                <i class="<?= esc($lay['icon']) ?>"></i>
                                            </div>
                                            <div>
                                                <strong class="text-dark d-block fs-6"><?= esc($lay['nama_layanan']) ?></strong>
                                                <small class="text-muted d-block text-truncate" style="max-width: 380px;"><?= esc($lay['deskripsi']) ?></small>
                                            </div>
                                        </div>
                                    </td>
                                    <td>
                                        <?php if (!empty($lay['nama_pj'])) : ?>
                                            <strong class="text-dark d-block small"><i class="fa-solid fa-user-tie text-secondary me-1"></i><?= esc($lay['nama_pj']) ?></strong>
                                        <?php endif; ?>
                                        <?php if (!empty($lay['kontak_wa'])) : ?>
                                            <a href="https://api.whatsapp.com/send?phone=<?= preg_replace('/[^0-9]/', '', $lay['kontak_wa']) ?>" target="_blank" class="text-success small text-decoration-none">
                                                <i class="fa-brands fa-whatsapp"></i> <?= esc($lay['kontak_wa']) ?>
                                            </a>
                                        <?php else : ?>
                                    ...
                                    <td class="text-center">
                                        <a href="<?= base_url('dashboard/pelayanan?layanan_id=' . $lay['id']) ?>" class="badge bg-primary bg-opacity-10 text-primary border border-primary border-opacity-25 px-2.5 py-1 rounded-pill text-decoration-none font-heading">
                                            <?= esc($lay['total_pelayanan']) ?> Kasus
                                        </a>
                                    </td>
                                    <td class="text-center">
                                        <?php if ($lay['status'] === 'aktif') : ?>
                                            <span class="badge bg-success-subtle text-success border border-success-subtle px-2.5 py-1 rounded-pill">Aktif di Web</span>
                                        <?php else : ?>
                                            <span class="badge bg-secondary-subtle text-secondary border border-secondary-subtle px-2.5 py-1 rounded-pill">Nonaktif</span>
                                        <?php endif; ?>
                                    </td>
                                    <td class="text-center">
                                        <div class="d-flex align-items-center justify-content-center gap-1.5">
                                            <a href="<?= base_url('dashboard/layanan/edit/' . $lay['id']) ?>" class="btn-action btn-edit" title="Ubah Layanan">
                                                <i class="fa-solid fa-pencil"></i>
                                            </a>
                                            <a href="<?= base_url('dashboard/layanan/delete/' . $lay['id']) ?>" class="btn-action btn-delete" onclick="return confirm('Hapus master layanan ini?')" title="Hapus Layanan">
                        ...
                        <?php else : ?>
                            <tr>
                                <td colspan="6" class="text-center py-5 text-muted">
                                    <i class="fa-solid fa-layer-group fs-1 mb-2 d-block text-secondary"></i>
                                    Belum ada master layanan masjid.
                                </td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
