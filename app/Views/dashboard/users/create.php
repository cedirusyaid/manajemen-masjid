...
<head>
    ...
    <title>Tambah Pengguna Baru - <?= site_name() ?></title>
...
<body>
    ...
    <main class="main-content">
        ...
        <div class="topbar">
            <div>
                <h1 class="h3 fw-bold mb-1 text-dark">Tambah Pengguna Baru</h1>
                <p class="text-muted mb-0">Daftarkan akun login untuk pengurus, panitia, atau staf pengelola.</p>
            </div>
        ...
        <div class="panel-card" style="max-width: 800px;">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h5 class="fw-bold mb-0 text-dark"><i class="fa-solid fa-user-plus me-2 text-success"></i>Form Registrasi Akun Pengguna</h5>
            ...
            <form action="<?= base_url('dashboard/users/store') ?>" method="POST">
                ...
                <div class="row g-3 mb-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" name="username" class="form-control" value="<?= old('username') ?>" placeholder="Contoh: bendahara_masjid" required autocomplete="off">
                        <small class="text-muted">Hanya huruf, angka, garis bawah (tanpa spasi).</small>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Email Resmi <span class="text-danger">*</span></label>
                        <input type="email" name="email" class="form-control" value="<?= old('email') ?>" placeholder="contoh@masjidagung.or.id" required>
                    </div>
                </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="personil_id" class="form-select">
                            ...
                            <?php foreach ($personils as $p) : ?>
                                <option value="<?= $p['id'] ?>" <?= old('personil_id') === $p['id'] ? 'selected' : '' ?>>
                                    <?= esc($p['nama']) ?> (<?= esc($p['no_hp'] ?: 'Tanpa No HP') ?>)
                                </option>
                            <?php endforeach; ?>
                        </select>
                        <small class="text-muted">Hubungkan akun ini dengan data pengurus/imam/khatib yang sudah ada.</small>
                    </div>
                    <div class="col-md-6">
                        ...
                        <select name="status" class="form-select" required>
                            ...
                            <option value="inactive" <?= old('status') === 'inactive' ? 'selected' : '' ?>>Non-Aktif (Terkunci)</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="p-3 bg-light rounded-3 mb-4 border border-light-subtle">
                    <div class="d-flex align-items-center gap-2 mb-2">
                        ...
                        <h6 class="fw-bold text-dark mb-0">Penugasan Kepanitiaan / Proyek (Opsional)</h6>
                    </div>
                    <p class="text-muted small mb-3">Pilih satu atau beberapa kepanitiaan yang menjadi wewenang akun ini (misal Bendahara/Panitia Pembangunan, Panitia Ramadhan, dsb).</p>
                            ...
                            <?php foreach ($kegiatans as $keg) : ?>
                                <div class="col-md-6">
                                    <div class="form-check p-2 border rounded bg-white shadow-none">
                                        <input class="form-check-input ms-1 me-2" type="checkbox" name="kegiatan_ids[]" value="<?= $keg['id'] ?>" id="keg_<?= $keg['id'] ?>" <?= in_array($keg['id'], old('kegiatan_ids', [])) ? 'checked' : '' ?>>
                ...
                <div class="d-flex justify-content-end gap-2 pt-2 border-top">
                    ...
                    <button type="submit" class="btn btn-submit">
                        <i class="fa-solid fa-save me-2"></i> Simpan Pengguna
                    </button>
                </div>
            </form>
        </div>
