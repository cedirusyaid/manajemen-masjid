...
<head>
    ...
    <title>Tambah Jadwal Shalat Manual - <?= site_name() ?></title>
...
<body>
    ...
    <main class="main-content">
        ...
        <div class="topbar">
            <div>
                <h1 class="h3 fw-bold mb-1 text-dark">Tambah Jadwal Waktu Shalat</h1>
                <p class="text-muted mb-0">Input waktu shalat khusus atau tanggal baru secara manual.</p>
            </div>
        ...
        <div class="panel-card" style="max-width: 800px;">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h5 class="fw-bold mb-0 text-dark"><i class="fa-solid fa-pen-to-square me-2 text-success"></i>Form Entri Jadwal Sholat</h5>
                <a href="<?= base_url('dashboard/jadwal-sholat') ?>" class="btn btn-outline-secondary rounded-pill px-3 py-1 btn-sm">
                    <i class="fa-solid fa-arrow-left me-1"></i> Kembali
                </a>
            </div>

            <form action="<?= base_url('dashboard/jadwal-sholat/store') ?>" method="POST">
                <?= csrf_field() ?>
                ...
                <div class="row g-3 mb-4">
                    <div class="col-md-6">
                        ...
                        <select name="bulan" class="form-select" required>
                            <?php foreach ($nama_bulan as $num => $nama) : ?>
                                <option value="<?= $num ?>" <?= (int)old('bulan', date('n')) === $num ? 'selected' : '' ?>>
                                    <?= $nama ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="tanggal" class="form-select" required>
                            <?php for ($d = 1; $d <= 31; $d++) : ?>
                                <option value="<?= $d ?>" <?= (int)old('tanggal', date('j')) === $d ? 'selected' : '' ?>>
                                    Tanggal <?= sprintf('%02d', $d) ?>
                                </option>
                            <?php endfor; ?>
                        </select>
                    </div>
                ...
                <div class="p-3 bg-light rounded-3 mb-4">
                    <h6 class="fw-bold text-dark mb-3"><i class="fa-solid fa-clock text-success me-2"></i>Rincian Waktu Shalat (Format HH:MM)</h6>
                    <div class="row g-3">
                        <div class="col-6 col-md-3">
                            <label class="form-label small">Subuh <span class="text-danger">*</span></label>
                            <input type="time" name="subuh" class="form-control" value="<?= old('subuh', '04:36') ?>" required>
                        </div>
                        <div class="col-6 col-md-3">
                            <label class="form-label small">Dzuhur <span class="text-danger">*</span></label>
                            <input type="time" name="dzuhur" class="form-control" value="<?= old('dzuhur', '12:09') ?>" required>
                        </div>
                        <div class="col-6 col-md-3">
                            <label class="form-label small">Ashar <span class="text-danger">*</span></label>
                            <input type="time" name="ashar" class="form-control" value="<?= old('ashar', '15:32') ?>" required>
                        </div>
                        <div class="col-6 col-md-3">
                            <label class="form-label small">Maghrib <span class="text-danger">*</span></label>
                            <input type="time" name="maghrib" class="form-control" value="<?= old('maghrib', '18:20') ?>" required>
                        </div>
                        <div class="col-6 col-md-3">
                            <label class="form-label small">Isya <span class="text-danger">*</span></label>
                            <input type="time" name="isya" class="form-control" value="<?= old('isya', '19:31') ?>" required>
                        </div>
                        <div class="col-6 col-md-3">
                            <label class="form-label small">Imsak (Opsional)</label>
                            <input type="time" name="imsak" class="form-control" value="<?= old('imsak') ?>" placeholder="Auto: Subuh - 10m">
                        </div>
                        <div class="col-6 col-md-3">
                            <label class="form-label small">Terbit (Opsional)</label>
                            <input type="time" name="terbit" class="form-control" value="<?= old('terbit') ?>" placeholder="Auto: Subuh + 75m">
                        </div>
                        <div class="col-6 col-md-3">
                            <label class="form-label small">Dhuha (Opsional)</label>
                            <input type="time" name="dhuha" class="form-control" value="<?= old('dhuha') ?>" placeholder="Auto: Terbit + 25m">
                        </div>
                    </div>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="keterangan" class="form-control" value="<?= old('keterangan') ?>" placeholder="Contoh: Rujukan Hisab / Penetapan Kemenag">
                </div>

                <div class="d-flex justify-content-end gap-2">
                    <a href="<?= base_url('dashboard/jadwal-sholat') ?>" class="btn btn-light px-4">Batal</a>
